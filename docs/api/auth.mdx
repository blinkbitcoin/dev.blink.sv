...
Use the same credentials as with the [Blink wallet app](https://gt.blink.sv).
...
* for your custom requests set the API key in the header as:
  ...
  "X-API-KEY" "blink_..."
  ```
* visit the GraphQL Playground at [api.blink.sv/graphql](https://api.blink.sv/graphql)
...
:::note
To use the legacy authentication token visit the [/api/legacy/auth](/api/legacy/auth) folder.
:::
