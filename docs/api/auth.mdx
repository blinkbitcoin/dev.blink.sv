...
* Note that there are methods to create invoices without authentication, but a more strict rate limiting is applied.
...
* The BTCPay connection strings contain the same API key togethet with the  ID of the chosen wallet in the format ready to be pasted in the BTCPay Server interfac.
